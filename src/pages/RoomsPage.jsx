            ...
            roomsState && roomsState.map(room => {
              return (
                <tr key={room.id} onClick={() => handleRoom(room.id)}>
                  ...
                  <td>
                    <p className={room.offer ? 'withOffer' : 'withoutOffer'}>{room.price / 100 + '$'}</p>
                  </td>
                  <td>
                    <p className={room.offer ? 'offer' : 'normal'}>{room.offer ? (room.price / 100 * (1 - room.discount / 100)).toFixed(2) + '$' : room.price / 100 + '$'}</p>
                  </td>
                </tr>
              )
            })
